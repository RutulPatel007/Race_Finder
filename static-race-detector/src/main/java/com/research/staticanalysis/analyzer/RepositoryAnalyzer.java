...
public class RepositoryAnalyzer {
    ...
    /**
     * Scans all Java files under root for Spring Data repository interfaces.
     * @return Map of RepositoryClassName -> EntityClassName
     *         e.g. "OrderRepository" -> "Order"
     */
                    ...
                    if (isSpringDataRepository(extendedType.getNameAsString())) {
                        if (extendedType.getTypeArguments().isPresent()) {
                            ...
                            if (!typeArgs.isEmpty()) {
                                ...
                                System.out.println("  [REPO] " + cid.getNameAsString() + " -> " + entityName);
                            }
                        }
                    }
                }
    ...
    private boolean isSpringDataRepository(String name) {
        return name.equals("JpaRepository") || 
               ...
               name.equals("ReactiveCrudRepository") ||
               name.equals("ReactiveMongoRepository") ||
               name.equals("R2dbcRepository");
    }
}